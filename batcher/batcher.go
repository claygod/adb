...
/*
Воркер можно организовать и по другому, без бесконечного цикла. Для этого надо
после собирания логов (их нужно собирать в строку) и перед сохранением на диск
запускать следующий цикл горутиной. Единственный нюанс, перед сохранением на диск
нужно делать проверку что предыдущее сохранение закончено - через переменную
с помощью атомик.

Тут у wal не нужна функция логирования (собирание строк), а требуется только
функция сохранения. Вот хронологический порядок:

for {batch<- log} | if !stop {go worker} | if x {save}
                   for {batch<- log} | if !stop {go worker} | if x {save}
				                       . . .
*/
